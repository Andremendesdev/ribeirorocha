    ...
    <>
      <motion.header
        ...
        className={`fixed top-0 inset-x-0 z-50 transition-all duration-500
          ${scrolled
            ? "bg-cream-50/92 backdrop-blur-xl shadow-warm-md border-b border-gold-200/50 py-3"
            : "bg-transparent py-5"
          }`}
      >
        <div className="max-w-7xl mx-auto px-6 lg:px-10 flex items-center justify-between">
          ...
          <a href="#inicio" className="flex items-center gap-3 group flex-shrink-0">
            <div className="relative w-10 h-10">
              ...
              {/* Inner fill */}
              <div className="absolute inset-[3px] rounded-full bg-gradient-to-br from-rose-700 to-rose-800 flex items-center justify-center">
                <span
                  className="text-[13px] font-semibold text-gold-300 leading-none"
          ...
          <button
            ...
            className="lg:hidden p-2.5 rounded-xl text-warm-700 hover:bg-cream-200 transition-colors"
          >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed inset-x-4 top-[76px] z-40 rounded-2xl
            ...
            <div className="p-5 space-y-1">
              {navLinks.map((link, i) => (
                <motion.a
                  ...
                  className="block px-4 py-3 rounded-xl text-sm font-medium text-warm-700
                    hover:text-rose-700 hover:bg-rose-700/5 transition-all"
                >
                  {link.label}
                </motion.a>
              ))}
              <div className="pt-3 border-t border-gold-200/50">
                <a
                  href="#contato"
                  onClick={() => setOpen(false)}
                  className="flex items-center justify-center gap-2 py-3 rounded-xl
                    bg-rose-700 text-white text-sm font-medium"
                >
                  <CalendarDays size={14} />
                  Agendar Consulta
                </a>
